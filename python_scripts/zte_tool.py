...
class zteRouter:
    ...
    def getCrVersion(self):
        header = {"Referer": self.referer}
        payload = "isTest=false&cmd=cr_version"
        r = requests.get(self.referer + f"goform/goform_get_cmd_process?{payload}", headers=header , data=payload)
    ...
    def getCookie(self):
        ...
        md5_rd0 = js2py.eval_js(newjs)
        RD = self.get_RD()
        newjs = self.js.replace("evalString", '"' + md5_rd0 + RD + '"')
        AD = js2py.eval_js(newjs)

        header = {"Referer": self.referer}
        hashPassword = self.hash(self.password).upper()
        ztePass = self.hash(hashPassword+LD).upper()
        payload = "isTest=false&goformId=LOGIN_MULTI_USER&user=" + self.user + "&password=" + ztePass
...
zteInstance = zteRouter("192.168.8.1", "admin", "PASSWORD")
